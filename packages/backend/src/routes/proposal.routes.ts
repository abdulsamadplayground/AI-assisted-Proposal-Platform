...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const proposals = await proposalService.listProposals(DEFAULT_USER_ID, req.query);
    res.json(proposals);
  } catch (error) {
    next(error);
  }
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const proposal = await proposalService.getProposalById(req.params.id, DEFAULT_USER_ID);
    res.json(proposal);
  } catch (error) {
    next(error);
  }
...
router.post('/:id/regenerate', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const proposal = await proposalService.regenerateProposal(
      req.params.id,
      DEFAULT_USER_ID,
...
router.post('/:id/submit', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const proposal = await proposalService.submitForApproval(req.params.id, DEFAULT_USER_ID);
    res.json(proposal);
  } catch (error) {
    next(error);
  }
...
router.post('/:id/approve', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const proposal = await proposalService.approveProposal(req.params.id, DEFAULT_USER_ID);
    res.json(proposal);
  } catch (error) {
    next(error);
  }
...
router.get('/:id/versions', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const versions = await proposalService.getProposalVersions(req.params.id, DEFAULT_USER_ID);
    res.json(versions);
  } catch (error) {
    next(error);
  }
...
router.get('/:id/export', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { buffer, filename } = await proposalService.exportProposalToWord(
      req.params.id,
      DEFAULT_USER_ID
    );
...
router.delete('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await proposalService.deleteProposal(req.params.id, DEFAULT_USER_ID);
    res.json(result);
  } catch (error) {
    next(error);
  }
